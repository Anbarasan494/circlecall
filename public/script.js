// ─────────────────────────────────────────────────────────────────────────────
//  CircleCall — client (all bugs fixed)
// ─────────────────────────────────────────────────────────────────────────────
...
// FIX: peers map stores { call, video } so we can remove tiles on disconnect
const peers        = {}   // peerId → { call, video }
const usernames    = {}   // peerId → display name
const mediaState   = {}   // peerId → { micOn, camOn }
const pendingCalls = []   // calls that arrived before myStream was ready
...
// FIX: Queue incoming calls if stream isn't ready yet, answer them once it is
myPeer.on("call", call => {
...
function answerCall(call) {
  ...
  // FIX: use makeVideoEl() so autoplay + playsInline are always set
  const video = makeVideoEl()

  // FIX: store in peers map so we can remove this tile on disconnect
  if (!peers[call.peer]) peers[call.peer] = { call, video }

  call.on("stream", remoteStream => {
    // Guard: only add tile once per peer
    if (document.querySelector(`[data-uid="${call.peer}"]`)) return
    peers[call.peer].video = video
...
function makeVideoEl(muted = false) {
  ...
  v.autoplay    = true   // FIX: always set autoplay
  v.playsInline = true   // FIX: required for iOS Safari
  v.muted       = muted
  return v
}
...
// ─────────────────────────────────────────────────────────────────────────────
//  enterRoom — called from the name modal
// ─────────────────────────────────────────────────────────────────────────────
function enterRoom() {
  const inp  = document.getElementById("name-input")
  const name = inp.value.trim()
  if (!name) { inp.style.outline = "2px solid #ef4444"; inp.focus(); return }
  ...
  navigator.mediaDevices.getUserMedia({ video: true, audio: true })
    .then(stream => {
      ...
      myVideo = makeVideoEl(true)   // muted=true: no local echo
      addVideoTile("local", myVideo, stream, username)

      streamReady = true

      // FIX: answer any calls that arrived before the stream was ready
      drainPendingCalls()

      // Report initial media state to server
      socket.emit("media-state", true, true)
...
function addVideoTile(id, video, stream, name) {
  // FIX: guard against duplicate tiles
  ...
  wrapper.dataset.uid = id   // FIX: data-uid so we can find & remove the tile
  wrapper.appendChild(video)

  // FIX: name tag on every tile
...
// FIX: properly removes the tile AND closes the peer call
...
// ─────────────────────────────────────────────────────────────────────────────
// FIX: no more unreliable setTimeout retry — stream is guaranteed ready here
// because callPeer is only triggered after existing-users/user-connected,
// which only fires after tryJoin(), which requires streamReady=true
function callPeer(id, name) {
  if (peers[id]) return
  if (!myStream)  { console.warn("callPeer: myStream null for", id); return }
...
// Existing users in room — sent once we join
socket.on("existing-users", (users, isHost, hId) => {
  amHost = isHost
  hostId = hId
  if (isHost) document.getElementById("host-badge-label").style.display = "inline-flex"

  // FIX: users is now an array of objects {id, username, micOn, camOn}
  // We also handle plain string IDs defensively (old server compat)
  users.forEach(u => {
    const uid   = (typeof u === "object") ? u.id       : u
    const uname = (typeof u === "object") ? u.username : (usernames[u] || u)
    usernames[uid]  = uname
...
// FIX: participant left — remove their tile
...
// Approved: tell server to finalise, it responds with existing-users
socket.on("approved", () => {
  hideWaitingScreen()
...
// Live participant list from server
...
socket.on("screen-share-started", sharerId => {
  ...
  // The sharer replaced the video track in the existing RTCPeerConnection.
  // The updated stream is already on their video element — mount the overlay.
  const peerVideo = peers[sharerId]?.video
  if (peerVideo?.srcObject) {
    mountScreenShareUI(peerVideo.srcObject, null, sharerId, usernames[sharerId])
  }
})
...
socket.on("connect_error", () => showToast("Reconnecting…", "error"))
...
function mountScreenShareUI(screenSrc, camSrc, sharerId, sharerName) {
  ...
  closeBtn.className = "ss-collapse"
  closeBtn.textContent = "✕ Exit fullscreen"
  closeBtn.onclick = unmountScreenShareUI
      ...
      <div style="display:flex;align-items:center;gap:10px;width:100%">
        ...
        <div class="p-info">
          ...
          <span class="p-status">
            ...
            <span title="Cam">${p.camOn  ? "📷" : "📵"}</span>
          </span>
        </div>
      </div>`

    // Host controls shown for every participant except self
    if (amHost && !p.isSelf) {
...
function appendMessage(sender, text, isSelf) {
  ...
  li.innerHTML = `
    <span class="msg-sender">${isSelf ? "You" : escapeHtml(sender)}</span>
...
// FIX: showToast was broken — it overwrote className instead of using classList
...
document.addEventListener("DOMContentLoaded", () => {
  // FIX: populate room code in the name modal
